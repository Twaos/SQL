...
course_descriprion nvarchar(50) NOT NULL
)
...
insert into Teacher(teacherID, teacher_name, courseID, email, classID)
values (1, 'Malle Marker', 2, 'marker@kool.ee', 1),
(2, 'Paul Pihlakas', 1, 'pihlakas@kool.ee', 2),
(3, 'Mari Maastik', 5, 'maastik@kool.ee', 3),
(4, 'Triin Tibu', 2, 'tibu@kool.ee', 1),
(5, 'Jüri Jüriöö', 3, 'jurioo@kool.ee', 2)

insert into Class(classID, teacherID, studentID, class_name, room_no, courseID)
values (1, 2, 1, '5A', '216', 2),
(2, 5, 2, '5B', '316', 5),
(3, 3, 3, '5C', '220', 1),
(4, 3, 5, '6A', '115', 2),
(5, 4, 4, '6B', '132', 3)

insert into Course(courseID, classID, course_name, course_descriprion)
values(1, 3, 'Matemaatika', 'reaalained'),
(2, 4, 'Ajalugu', 'reaalained'),
(3, 3, 'Kunst', 'humanitaarained'),
(4, 2, 'Geofraafia', 'reaalained'),
(5, 1, 'Bioloogia', 'reaalained')

insert into Student(studentID, classID, name, id_code, email, scoreID)
values (1, 2, 'Priit Lepp', 5100233, 'priit@gmail.com', 5),
(2, 1, 'Rainer Tamm', 4785654,'rainer@gmail.com', 3),
(3, 5, 'Signe Seep', 4569875,'signe@gmail.com', 5),
(4, 2, 'Kaspar Tuvi', 1478524,'kaspar@gmail.com', 2),
(5, 3, 'Pille Pirukas', 4563215,'pille@gmail.com', 4)

insert into Score(scoreID, score, studentID, teacherID,courseID,date)
values (1, 5, 1, 1, 1, '2026-05-05'),
(2, 3, 2, 5, 1, '2026-05-05'),
(3, 4, 3, 4, 1, '2026-05-05'),
(4, 4, 3, 1, 1, '2026-05-05'),
(5, 5, 1, 2, 1, '2026-05-05')


create procedure spTeachersEmail
as begin
	select teacher_name, email from Teacher
end
exec spTeachersEmail
select name, classID, score
from Student
left join Score
on Student.ScoreId = Score.ScoreId
select teacher_name, course_name, course_descriprion
from Teacher
join Course
on Teacher.courseID = course.courseID
select class_name, room_no, name
from Class
join Student
...
(
    galerii_id INT NOT NULL PRIMARY KEY,
    nimi NVARCHAR(100) NOT NULL,
    aadress NVARCHAR(200) NOT NULL,
    telefon NVARCHAR(20),
    email NVARCHAR(100)
);

-- 2. KUNSTNIK tabel
CREATE TABLE Kunstnik
(
    kunstnik_id INT NOT NULL PRIMARY KEY,
...
CREATE TABLE Teos
(
    teos_id INT NOT NULL PRIMARY KEY,
    kunstnik_id INT NOT NULL,
    pealkiri NVARCHAR(200) NOT NULL,
    loomise_aasta INT,
    tehnika NVARCHAR(100),
    mootmed NVARCHAR(50),
    olek NVARCHAR(50)
);
...
CREATE TABLE Naitus
(
    naitus_id INT NOT NULL PRIMARY KEY,
    galerii_id INT NOT NULL,
    pealkiri NVARCHAR(200) NOT NULL,
    algus_kuupaev DATE,
    lopp_kuupaev DATE,
    kuraator NVARCHAR(100),
    kirjeldus TEXT
);

-- 5. NAITUSE_TEOS (seoste tabel)
CREATE TABLE Naituse_Teos
(
    naituse_teos_id INT NOT NULL PRIMARY KEY,
    naitus_id INT NOT NULL,
    teos_id INT NOT NULL,
    eksponeerimise_kuupaev DATE
);

-- ANDMETE SISESTAMINE

-- Galerii andmed (5 rida)
INSERT INTO Galerii(galerii_id, nimi, aadress, telefon, email)
VALUES 
(1, 'Avatud Ruum Tallinn', 'Raekoja plats 1, Tallinn', '+372 5551234', 'tallinn@avatudruum.ee'),
(2, 'Avatud Ruum Tartu', 'Ülikooli tn 2, Tartu', '+372 5555678', 'tartu@avatudruum.ee'),
(3, 'Avatud Ruum Pärnu', 'Rüütli tn 5, Pärnu', '+372 5559012', 'parnu@avatudruum.ee'),
(4, 'Avatud Ruum Narva', 'Vabaduse tn 8, Narva', '+372 5553456', 'narva@avatudruum.ee'),
(5, 'Avatud Ruum Kuressaare', 'Lossi tn 3, Kuressaare', '+372 5557890', 'kuressaare@avatudruum.ee');

-- Kunstnik andmed (5 rida)
INSERT INTO Kunstnik(kunstnik_id, eesnimi, perekonnanimi, synniaasta, rahvus, elulugu)
VALUES 
(1, 'Mari', 'Mägi', 1985, 'eestlane', 'Mari Mägi on tuntud abstraktse maalikunstnik.'),
(2, 'Jaan', 'Tamm', 1972, 'eestlane', 'Jaan Tamm on skulptor, kelle teosed on inspireeritud loodusest.'),
(3, 'Liisa', 'Kull', 1990, 'eestlane', 'Liisa Kull on noor ja innovatiivne kunstnik.'),
(4, 'Peeter', 'Raud', 1968, 'eestlane', 'Peeter Raud on fotograaf, kelle portfoolios on üle 3000 teose.'),
(5, 'Anna', 'Lemberg', 1980, 'rootslane', 'Anna Lemberg on rahvusvaheliselt tunnustatud kunstnik.');

-- Teos andmed (10 rida)
INSERT INTO Teos(teos_id, kunstnik_id, pealkiri, loomise_aasta, tehnika, mootmed, olek)
VALUES 
(1, 1, 'Sügise värvid', 2020, 'Õli lõuendil', '80x100 cm', 'saadaval'),
(2, 1, 'Mere hääl', 2021, 'Akvarell', '50x70 cm', 'müüdud'),
(3, 2, 'Tammepuu', 2019, 'Pronks', '120x80x40 cm', 'saadaval'),
(4, 2, 'Tuuleliiv', 2020, 'Kivi', '60x40x30 cm', 'välja laenatud'),
(5, 3, 'Digitaalne unenägu', 2022, 'Digitaalne trükk', '100x100 cm', 'saadaval'),
(6, 3, 'Linnurada', 2021, 'Segatehnika', '120x90 cm', 'müüdud'),
(7, 4, 'Talvine Tallinn', 2020, 'Must-valge foto', '40x60 cm', 'saadaval'),
(8, 4, 'Pärnu rannad', 2019, 'Värvifoto', '60x90 cm', 'saadaval'),
(9, 5, 'Skandinaavia valgus', 2021, 'Õli lõuendil', '100x120 cm', 'müüdud'),
(10, 5, 'Metsa varjud', 2022, 'Akrüül', '80x100 cm', 'saadaval');

-- Naitus andmed (5 rida)
INSERT INTO Naitus(naitus_id, galerii_id, pealkiri, algus_kuupaev, lopp_kuupaev, kuraator, kirjeldus)
VALUES 
(1, 1, 'Sügisesed toonid', '2023-09-01', '2023-10-15', 'Kadri Kask', 'Näitus, mis toob esile sügise värve.'),
(2, 1, 'Noored talent', '2023-11-01', '2023-12-20', 'Marek Mänd', 'Noorte kunstnike ühisnäitus.'),
(3, 2, 'Looduse ilu', '2023-10-01', '2023-11-30', 'Liisa Lill', 'Loodusest inspireeritud teoste näitus.'),
(4, 3, 'Rannavalge', '2023-06-01', '2023-08-31', 'Mati Meri', 'Suvenäitus Pärnus.'),
(5, 4, 'Põhjamaa hing', '2024-01-10', '2024-03-15', 'Eva Eha', 'Talvine näitus Narvas.');

-- Naituse_Teos andmed (13 rida)
INSERT INTO Naituse_Teos(naituse_teos_id, naitus_id, teos_id, eksponeerimise_kuupaev)
VALUES 
(1, 1, 1, '2023-09-01'),
(2, 1, 2, '2023-09-05'),
(3, 1, 10, '2023-09-10'),
...
-- STORED PROCEDURE (1)
CREATE PROCEDURE spKunstnikudJaTeosed
AS BEGIN
    SELECT eesnimi, perekonnanimi, pealkiri, loomise_aasta
    FROM Kunstnik
    JOIN Teos
    ON Kunstnik.kunstnik_id = Teos.kunstnik_id
END

-- Stored procedure kutsumine
EXEC spKunstnikudJaTeosed;

-- 3 LIHTSAT JOIN PÄRINGUT

-- JOIN 1: Kunstnikud ja nende teosed
SELECT eesnimi, perekonnanimi, pealkiri, loomise_aasta
FROM Kunstnik
JOIN Teos
ON Kunstnik.kunstnik_id = Teos.kunstnik_id

-- JOIN 2: Näitused ja galeriid
SELECT Naitus.pealkiri, Galerii.nimi, Galerii.aadress, Naitus.algus_kuupaev
FROM Naitus
JOIN Galerii
ON Naitus.galerii_id = Galerii.galerii_id

-- JOIN 3: Teosed ja nende näitused
SELECT Teos.pealkiri, Naitus.pealkiri, Naituse_Teos.eksponeerimise_kuupaev
FROM Teos
JOIN Naituse_Teos
ON Teos.teos_id = Naituse_Teos.teos_id
JOIN Naitus
ON Naituse_Teos.naitus_id = Naitus.naitus_id